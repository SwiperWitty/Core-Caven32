...
char Pick_Agreement(struct Caven_Data *source, struct _Agreement *Item ,struct _Function *Fun,char debug)
{
    ...
    if (source->Length != 0)
    {
        if(source->Buff[0] == 0x5A)
        {
            if (source->Length >= 8 && Item->Flag == 0)
            {
                Item->Head = source->Buff[0];
                for (i = 1; i <= 4; i++)
                {
                    temp_32 = temp_32 << 8;
                ...
                if(source->Buff[3] & 0x20)           //RS485                    //长(1)
                {
                    Item->Control.RS485_Flag = 1;
                    Item->Addr = source->Buff[5];
                    Item->Length = ((int)source->Buff[6] << 8) + source->Buff[7];
                    Item->Flag = 8;
                }
                else                                                                //短(1)
                {
                    ...
                    Item->Addr = 0;
                    Item->Length = ((int)source->Buff[5] << 8) + source->Buff[6];     
                    Item->Flag = 7;
                }
                if(Item->Length >= 1024)
                {
                ...
                Item->Control.Model = source->Buff[1];
                Item->Control.Version = source->Buff[2];        //31-24
                Item->Control.M_or_S = (U8)(source->Buff[3] & 0X10) >> 4;       //(2)
                Item->Control.Class = (source->Buff[3] & 0X0F);                 //(3)
                Item->Control.MID = source->Buff[4];            //7-0
                Item->pointer = &Item->Buff[Item->Flag];
            }
            else if(Item->Flag != 0 && source->Length >= Item->num)
            {
                memcpy(Item->Buff,source->Buff,source->Length);
                Item->Check =  ((int)source->Buff[Item->Length + Item->Flag] << 8) + source->Buff[Item->Flag + Item->Length + 1];
                i = CRC16_CCITT_CalculateBuf(&(source->Buff[1]), Item->num - 3);      //除去头帧和 check位
                if (Item->Check != i)
                {
                ...
                N = Find_Data(source->Buff + Item->num,0x5A,10);
                if (N != 0)
                {
                    source->Length = source->Length - Item->num;
                    memcpy(source->Buff,Item->Buff + (Item->num + N - 1),source->Length);       //别动 !!!!
//                    string[0] = 0xff;string[1] = 0xff;
//                    Fun->Send_Data(4,string,2);
//                    Fun->Send_Data(4,source->Buff,source->Length);
//                    Fun->Send_Data(4,string,2);
                    //对source重新赋值，重置 Length
            ...
            else if (source->Length != 0)           //存在 source->Buff[0] == 0x5A ，且不能按完全接收，且串口没有数据接收之后
            {
                if(RX_Get_Num == source->Length)        //接收数据没有跳动
                {
                    Fun->Delay(5);
                    if (RX_Get_Num == source->Length)   //确认没有
                    {
