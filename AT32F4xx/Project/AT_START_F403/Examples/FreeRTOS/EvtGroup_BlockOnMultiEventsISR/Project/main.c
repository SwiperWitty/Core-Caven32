/**
  ******************************************************************************
  * File   : FreeRTOS/EvtGroup_BlockOnMultiEventsISR/main.c 
  * Version: V1.2.7
  * Date   : 2020-11-10
...
static void vTaskLED(void *pvParameters)
{
    ...
    while(1)
    {
		...
		uxBits = xEventGroupWaitBits(xHandleEventGroupPrint,
		                                BIT_0 | BIT_1,//The bits 0 and 1 within the event group to wait for
		                                pdTRUE,//Bits 0 and 1 should be cleared before returning
		                                pdTRUE,//Wait for both bits
		                                xTicksToWait);
		if((uxBits & (BIT_0 | BIT_1)) == (BIT_0 | BIT_1))
		{
		    App_Printf("\r\nbit 0 and 1 both are set, and the synchronization is success\r\n");
		}
...
static void vTaskMsgProcess(void *pvParameters)
{
    ...
    App_Printf("2. Set bit 0 of event group in ISR\r\n");
    App_Printf("3. Set bit 1 of event group in ISR\r\n");
    while(1)
    {
...
static void TMR_CallBack1(void)
{
    ...
    /* xEventGroupSetBitsFromISR sends a message to the RTOS daemon task to
     * have the set operation performed in the context of the daemon task */
    xResult = xEventGroupSetBitsFromISR(xHandleEventGroupPrint, BIT_0, &xHigherPriorityTaskWoken);

    if(xResult != pdFAIL)//The message was posted successfully
    {
...
static void TMR_CallBack2(void)
{
    ...
    xResult = xEventGroupSetBitsFromISR(xHandleEventGroupPrint, BIT_1, &xHigherPriorityTaskWoken);
