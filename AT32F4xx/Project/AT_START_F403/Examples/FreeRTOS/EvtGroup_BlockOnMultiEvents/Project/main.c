/**
  ******************************************************************************
  * File   : FreeRTOS/EvtGroup_BlockOnMultiEvents/main.c 
  * Version: V1.2.7
  * Date   : 2020-11-10
...
/** @addtogroup FreeRTOS_EvtGroup_BlockOnMultiEvents
  * @{
  */ 
#define BIT_0 (1<<0)
#define BIT_1 (1<<1)
...
static void vTaskMsgProcess(void *pvParameters)
{
    ...
    App_Printf("2. Set bit 0 of event group\r\n");
    App_Printf("3. Set bit 1 of event group\r\n");
    while(1)
    {
        ...
        switch(ucCommandValue)
        {
            ...
            case '2':
                ...
                uxBits = xEventGroupSetBits(xHandleEventGroupPrint, BIT_0);//set bit 0
                if(uxBits & BIT_0)
                {
                    App_Printf("\r\nBit 0 of event group is set\r\n");
                }
                else
                {
                    App_Printf("\r\nBit 0 of event group is cleared by other task\r\n");
                }
                break;
            case '3':
                //bsp_StartHardTimer(2, 50000, (void*)TMR_CallBack2);//after 50ms to execute ISR, and then disable CC2 interrupt
                uxBits = xEventGroupSetBits(xHandleEventGroupPrint, BIT_1);//set bit 1
                if(uxBits & BIT_1)
                {
                    App_Printf("\r\nBit 1 of event group is set\r\n");
                }
                else
                {
                    App_Printf("\r\nBit 1 of event group is cleared by other task\r\n");
                }
                break;
            default:
                break;
