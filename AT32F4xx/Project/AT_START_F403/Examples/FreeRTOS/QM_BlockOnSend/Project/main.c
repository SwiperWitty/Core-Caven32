/**
  ******************************************************************************
  * File   : FreeRTOS/QM_BlockOnSend/main.c 
  * Version: V1.2.7
  * Date   : 2020-11-10
...
/** @addtogroup FreeRTOS_QM_BlockOnSend
  * @{
  */ 
...
/**
 * @brief   Send the value to the queue
 * @param   pvParameters:passed to the task
 * @retval  None
 */
static void vTaskSender(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        xStatus = xQueueSendToBack(xHandleQueue, pvParameters, pdMS_TO_TICKS(100));
        if(xStatus != pdPASS)
        {
...
static void vTaskReceiver(void *pvParameters)
{
    ...
    while(1) 
    {
         ...
         * will only enter the Blocked state when the queue is full so this
         * task always expects the number of items in the queue to be equal
         * to the queue length, which is 3 in this case */

        if(uxQueueMessagesWaiting(xHandleQueue) != 3)
        {
            App_Printf("Queue should have been full\r\n");
        }
        
        /* Receive data from the queue */
        xStatus = xQueueReceive(xHandleQueue, &xReceivedStructure, 0);
        if(xStatus != pdPASS)
        {
        ...
        else
        {
            ...
            {
                App_Printf("From Sender 1 = %d\r\n", xReceivedStructure.ucValue);
            }
            else
            {
                App_Printf("From Sender 2 = %d\r\n", xReceivedStructure.ucValue);
            }
        }
    }
}
...
static void AppObjCreate(void)
{
    ...
    /* The queue is created to hold a maximum of 3 structures
     * of type Data_t */
    xHandleQueue = xQueueCreate(3,sizeof(Data_t));
