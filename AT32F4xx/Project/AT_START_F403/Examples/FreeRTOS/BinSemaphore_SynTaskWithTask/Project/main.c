/**
  ******************************************************************************
  * File   : FreeRTOS/BinSemaphore_SynTaskWithTask/main.c 
  * Version: V1.2.7
  * Date   : 2020-11-10
...
static SemaphoreHandle_t xHandleBinarySemaphore = NULL;
...
static void vTaskMsgProcess(void *pvParameters)
{
    ...
    App_Printf("2. Give the binary semaphore to the task\r\n");  
    ...
    while(1)
    {
        ...
        switch(ucCommandValue)
        {
            ...
            case '2':
                ...
                xSemaphoreGive(xHandleBinarySemaphore);
                break;
            case '3':
...
static void vTaskCtrolProcess(void *pvParameters)
{
    while(1)
    {
		 ...
		 */
	    xSemaphoreTake(xHandleBinarySemaphore, portMAX_DELAY);
	    App_Printf("\r\nThe Binary Semaphore is taked\r\n");
    }
}
...
static void AppObjCreate(void)
{
     ...
     */
    xHandleBinarySemaphore = xSemaphoreCreateBinary();
    if(xHandleBinarySemaphore == NULL)
    {
        printf("There is insufficient heap memory\r\n");
    }
}
...
static void TMR_CallBack1(void)
{
    ...
    xSemaphoreGiveFromISR(xHandleBinarySemaphore, &xHigherPriorityTaskWoken);
