/**
  ******************************************************************************
  * File   : FreeRTOS/CountSemaphore_SynTaskWithTask/main.c 
  * Version: V1.2.7
  * Date   : 2020-11-10
...
static SemaphoreHandle_t xHandleCountingSemaphore = NULL;
...
static void vTaskMsgProcess(void *pvParameters)
{
    ...
    App_Printf("2. Give the counting semaphore to the task\r\n");  
    ...
    while(1)
    {
        ...
        switch(ucCommandValue)
        {
            ...
            case '2':
                ...
                xSemaphoreGive(xHandleCountingSemaphore);
                xSemaphoreGive(xHandleCountingSemaphore);
                xSemaphoreGive(xHandleCountingSemaphore);
                break;
            case '3':
        ...
        if(ucCommandValue =='1' || ucCommandValue =='2' || ucCommandValue =='3')
        {
            ...
            App_Printf("2. Give the counting semaphore to the task\r\n"); 
            //App_Printf("2. Enable Timer CC1 interrupt to give binary semaphore\r\n");  
            //App_Printf("3. Enable Timer CC2 interrupt\r\n"); 
            comClearRxFifo(COM1);
            ucCommandValue = 0xFF;
        }
...
static void vTaskCtrolProcess(void *pvParameters)
{
    ...
    {
        vTaskDelay(pdMS_TO_TICKS(100));
		/* The task blocks indefinitely, so there is no need to 
		 * check the value returned by xSemaphoreTake()
		 */
	    xSemaphoreTake(xHandleCountingSemaphore, portMAX_DELAY);
	    App_Printf("\r\nThe counting Semaphore is taked\r\n");
    }
}
...
static void AppObjCreate(void)
{
     ...
     * meaning the semaphore must first be given before
     * it can be taken using the xSemaphoreTake() function
     */
    xHandleCountingSemaphore = xSemaphoreCreateCounting(10,0);
    if(xHandleCountingSemaphore == NULL)
    {
        printf("There is insufficient heap memory\r\n");
    }
}
...
static void TMR_CallBack1(void)
{
    ...
    xSemaphoreGiveFromISR(xHandleCountingSemaphore, &xHigherPriorityTaskWoken);
