/**
  ******************************************************************************
  * File   : FreeRTOS/ResManagement_MutexesAndTask/main.c 
  * Version: V1.2.7
  * Date   : 2020-11-10
...
static void vTaskCtrolProcess1(void *pvParameters);
static void vTaskCtrolProcess2(void *pvParameters);
...
static TaskHandle_t xHandleTaskCtrolProcess1    = NULL;
static TaskHandle_t xHandleTaskCtrolProcess2    = NULL;
...
/**
 * @brief   Use mutex semaphore to protect print function
 * @param   pvParameters: passed to the task
 * @retval  None
 */
static void vTaskCtrolProcess1(void *pvParameters)
{
    ...
    while(1)
    {
        counter++;//This is a fast operation
        xSemaphoreTake(xHandleMutexPrint, portMAX_DELAY);//Obtain the mutex
        xTimeAtWhichMutexWasTaken = xTaskGetTickCount();//Record the time at which the mutex was taken
        printf("This is Ctrol Task1: %d\r\n", counter);//this is a slow operation
        xSemaphoreGive(xHandleMutexPrint);//Give back the mutex
         ...
         * Therefore, only call taskYIELD() if the tick count changed while the mutex was held
         */
...
static void AppTaskCreate (void)
{
	...
	xReturn = xTaskCreate(  vTaskCtrolProcess1,     		
                            "vTaskCtrolProcess1",   		
                            512,            		
                            NULL,           		
                            1,              		
                            &xHandleTaskCtrolProcess1 ); 
    if(xReturn != pdPASS)
	{
	    while(1);//There is insufficient heap memory
	}

	xReturn = xTaskCreate(  vTaskCtrolProcess2,     		
                            "vTaskCtrolProcess2",   		
                            512,            		
                            NULL,           		
                            1,              		
                            &xHandleTaskCtrolProcess2 ); 
    if(xReturn != pdPASS)
	{
	    while(1);//There is insufficient heap memory
	}
