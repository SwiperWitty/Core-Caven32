...
uint8_t sys_buff_a[500];
uint8_t sys_buff_b[500];
int sys_num_a = 0;
int sys_num_b = 0;
char sys_buff_flag = 0;
...
int Center_State_machine(Caven_BaseTIME_Type time)
{
...
#endif
	temp_num = Center_app_Dual_cache_Make_pack 
	(sys_buff_a,&sys_num_a,sys_buff_b,&sys_num_b,&sys_buff_flag,JSON_array,&JSON_len,sizeof(JSON_array),SYS_Link,time);
	if(temp_num == 2)
	{
	...
	if(JSON_len)
	{
		...
		{
			Mode_Use.UART.Send_Data_pFun(m_UART_CH1,JSON_array,JSON_len);
			JSON_len = 0;
		}
	}
...
void debug_info_handle (void *data)
{
	...
	else
	{
		if(sys_buff_flag == 0)
		{
			if (sys_num_a < sizeof(sys_buff_a))
			{
				sys_buff_a[sys_num_a++] = temp_data;
			}
		}
		else
		{
			if (sys_num_b < sizeof(sys_buff_b))
			{
				sys_buff_b[sys_num_b++] = temp_data;
			}
		}
	}
}
