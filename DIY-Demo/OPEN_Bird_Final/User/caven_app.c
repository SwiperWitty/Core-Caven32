    ...
    case m_CAVEN_CMD1_Model_Order:
        {
            ...
            {
	#if SYS_BTLD
				memcpy(&pack.p_Data[temp_run],"bootld",strlen("bootld"));
				temp_run += strlen("bootld");
	#else
				memcpy(&pack.p_Data[temp_run],DEMO_Name_str,strlen(DEMO_Name_str));
				temp_run += strlen(DEMO_Name_str);
	#endif
                pack.dSize = temp_run;
                pack.Result = 0;
            }
...
static Caven_BaseTIME_Type debug_time,rs232_time,rs485_time,server_time,client_time,net4g_time,udp_time,BLE_time;
int Caven_app_Make_pack (uint8_t data,int way,Caven_BaseTIME_Type time)
{
    ...
    case BLE_Link:
        {
            temp_pack = &Caven_packet_BLE;
            temp_num = time.SYS_Sec - BLE_time.SYS_Sec;
			BLE_time = time;
        }
        break;
    default:
    ...
    if (temp_pack != NULL && retval == 0)
    {
        ...
        if (retval == 0xFF)
        {
            ...
            Caven_Circular_queue_input (*temp_pack,Caven_packet_buff,6);   // 常规入队 
            Caven_info_packet_fast_clean_Fun(temp_pack);
        }
        else if (retval < 0)
        {
...
int Caven_app_JSON_Make_pack (char *data,int way)
{
	...
	if(temp_str != NULL)
	{
		...
		if (retval >= 0)
		{
			...
			Caven_Circular_queue_input (*temp_pack,Caven_packet_buff,6);   // JSON入队 
            Caven_info_packet_fast_clean_Fun(temp_pack);
		}
	}
	return retval;
